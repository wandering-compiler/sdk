...
func URLToDriverDSN(urlDSN string) (string, error) {
	...
	// NO_BACKSLASH_ESCAPES an apply-time literal (`\d` in an author's
	// regex CHECK, a DEFAULT, an enum value, a COMMENT) landed corrupted.
	// The runtime had carried that pin for a pass already; the applier was
	// a third copy nobody updated.
	//
	// A parameter the operator named themselves is left alone — these
	// refuse to inherit an ACCIDENTAL setting, they do not overrule a
	// chosen one.
	for _, p := range dbsession.MySQLDSNParams() {
