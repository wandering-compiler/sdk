...
// MountMetricsListener serves the Prometheus exporter on
// <PREFIX>_METRICS_PORT (when set). Returns the listener
// address, or "" when the env is unset OR the port could not
// be bound — so main.go's "metrics listening on …" line means
// metrics are actually being served. A bind failure is logged
// (operator-visible) but doesn't kill the main process.
//
// The bind happens SYNCHRONOUSLY, only the serve loop is a
// goroutine. It used to be the other way round, and the whole
// failure lived inside that goroutine: the caller was told the
// address unconditionally, logged that metrics were up, and the
// only trace of "address already in use" was one line the
...
func MountMetricsListener(prefix string, lookup func(string) string, log func(format string, args ...any)) string {
	...
	if err != nil {
		log("metrics listener on %s: %v — /metrics is NOT being served", addr, err)
		return ""
	}
