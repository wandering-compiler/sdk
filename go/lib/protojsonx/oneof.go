...
// scalarJSONKind classifies how the w17 dialect renders a scalar
// field's JSON value — "string", "number", or "boolean" — so a bare
// oneof arm can be matched back to its variant by type. 64-bit integers
// and bytes render as JSON strings; 32-bit ints, floats AND ENUMS as
// numbers. Message kinds return "" (they carry the discriminator
// instead).
//
// The enum arm is the one place this deviates from stock protojson:
// the w17 wire sends an enum's number (restgw.Marshaller sets
// UseEnumNumbers), so a bare enum arm arrives as a JSON number and must
// be classified as one. Leaving it under "string" would route an
// incoming enum to whichever string arm came first, or drop it.
//
// Consequence worth knowing: a oneof can carry at most one bare arm per
// JSON primitive (resolveArm takes the first match), so an enum arm and
// a 32-bit-int arm in the SAME oneof are now indistinguishable where
// they used to be tellable apart. Pair such an enum with a wrapper
// message arm — that one carries a discriminator.
