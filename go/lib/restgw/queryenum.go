...
// enumValueList renders the declared values for an error message as
// `NAME=number` pairs. Both spellings are accepted, and the NUMBER is
// what the JSON wire carries (restgw.Marshaller sets UseEnumNumbers),
// so a caller reading a response and then building a query needs to
// see the mapping — a list of bare names would leave them guessing why
// the `2` they just received is not in the accepted set.
func enumValueList(ed protoreflect.EnumDescriptor) string {
	values := ed.Values()
	out := make([]byte, 0, values.Len()*20)
	for i := 0; i < values.Len(); i++ {
		...
		out = append(out, '=')
		out = strconv.AppendInt(out, int64(v.Number()), 10)
	}
	return string(out)
}
