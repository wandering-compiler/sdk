...
// TestDownloadURLRoundTrip — INVARIANT: a URL this package BUILDS is a
// URL this package can SERVE.
//
// The two halves had only ever been tested apart. Every existing
// ServeDownload test hand-writes its request path, and BuildDownloadURL
// was checked against expected strings — so nothing ever asked whether
// the string one produces is the string the other accepts. It is not:
//
//	BuildKey         → "/task-attachments/6e/cb/<sha>"   (leading slash)
//	BuildDownloadURL → prefix + "/" + key                (so: prefix + "//" + …)
//	ServeDownload    → strips prefix + "/", leaving "/task-attachments/…"
//	matchBucket      → looks for "task-attachments/"     → no match → 404
//
// Which means every download URL the gateway hands a client 404s at the
// gateway's own handler. The write path and the read path disagreed
// about one slash, and both were green.
//
// This test is deliberately written as a round trip rather than as an
// assertion on either string: pinning the URL's expected text would just
// have recorded whichever side was consulted first.
func TestDownloadURLRoundTrip(t *testing.T) {
	const bucket = "/task-attachments"
	const prefix = "/api/v1/tasks/files"

	d := memory.New()
	key := kvfs.BuildKey(bucket, "deadbeef", 100000, 100)
	d.PutBytes(key, []byte("ROUND-TRIP-BODY"))

	cfg := restgw.NewDownloadConfig(
		prefix,
		[]restgw.DownloadField{{BucketPath: bucket, Driver: d}},
		"", false, false, false,
	)

	url := restgw.BuildDownloadURL("RELATIVE", prefix, "", key)
	if strings.Contains(url, "//") {
		t.Errorf("BuildDownloadURL produced %q — the empty path segment survives any proxy that does not merge slashes, and is collapsed by every one that does, so the URL means two different things depending on what is in front of the gateway", url)
	}
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("GET %s → %d, want 200.\nThe gateway cannot serve the URL it just rendered for this object.", url, rec.Code)
	}
	if got := rec.Body.String(); got != "ROUND-TRIP-BODY" {
		t.Errorf("body = %q, want ROUND-TRIP-BODY", got)
	}
}
