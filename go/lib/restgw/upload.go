...
type FilePartConfig struct {
	...
	// atomic on the same fs only).
	//
	// Leave it EMPTY and the helper asks the Driver where to
	// stage (kvfs.StagingDirProvider) — the driver is the
	// only component that knows its bucket root, which is a
	// boot-time env value no generator can bake in. Drivers
	// with no local filesystem advertise nothing and staging
...
func ProcessFilePart(ctx context.Context, r *http.Request, cfg FilePartConfig) (string, error) {
	...
	stageDir, err := resolveStagingDir(cfg)
	if err != nil {
		return "", err
	}

	tmpPath, hash, err := streamToTempFile(file, stageDir, cfg.MaxSizeBytes)
	if err != nil {
		return "", err
	}
...
// resolveStagingDir decides where the streaming write lands.
// An explicit TmpDir always wins (a hand-written caller that
// knows its own layout). Otherwise the driver is asked: a
// local-filesystem driver answers with a directory inside its
// bucket root, which makes the subsequent PutFromTempFile a
// same-filesystem rename — the only publish that is atomic by
// construction. A driver that advertises nothing yields "",
// which streamToTempFile resolves to os.TempDir().
//
// A driver that CAN'T produce its staging dir (unwritable bucket
// root) fails the upload here rather than silently staging
// somewhere that would need the non-atomic fallback — the same
// root cause would fail the put a moment later anyway, with a
// worse message.
func resolveStagingDir(cfg FilePartConfig) (string, error) {
	if cfg.TmpDir != "" {
		return cfg.TmpDir, nil
	}
	sp, ok := cfg.Driver.(kvfs.StagingDirProvider)
	if !ok {
		return "", nil
	}
	dir, err := sp.StagingDir()
	if err != nil {
		return "", fmt.Errorf("restgw: driver staging dir: %w", err)
	}
	return dir, nil
}
