...
// PermissionDenied three services from the real cause). This closes that
...
//
// A composed binary reaches storage in-process and never dials, so this
// interceptor does not run there — service/inprocgrpc applies the same
// relay itself at its boundary (see its serverContext). It used to need no
// relay because the in-process handler simply inherited the caller's
// context, but inheriting the WHOLE context also carried tx routing into
