...
func (st *loadState) pickIdx() int {
	...
	if st.plan.Selection == SelectRandom {
		// #nosec G404 -- picks WHICH load step to run next under a random
		// selection plan. Nothing about the choice is a secret; an
		// unpredictable-to-an-attacker sequence is not a property a load
		// generator needs.
		return rand.IntN(n)
	}
