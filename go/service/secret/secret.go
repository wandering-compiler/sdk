...
// The write error is discarded because [fmt.Formatter] has no error
// return: fmt itself swallows sink errors, so there is nowhere to
// report one and nothing a redaction routine could do about it.
func (s Secret[T]) Format(f fmt.State, _ rune) { _, _ = io.WriteString(f, Redacted) }
