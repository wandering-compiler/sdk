...
// local filesystem advertises: "stage uploads HERE and my
// PutFromTempFile stays a same-filesystem rename".
//
// It exists because atomicity is a property of the PAIR (staging
// location, bucket) and only the driver knows the second half. The
// bucket root is a runtime value — an env var the gateway resolves at
...
type StagingDirProvider interface {
	// StagingDir returns an existing directory on the same filesystem
	// as the driver's storage, created on demand. Callers write their
	// own temp files there and are responsible for removing them.
...
func BuildKeyAt(bucketPath, objectKey string, depth int) string {
	...
	if depth <= 0 {
		...
		return strings.TrimPrefix(prefix, "/") + "/" + objectKey
	}
	...
	// No leading slash. That is the form every driver and every caller
	// uses — the download read path reconstructs a key without one
	// (restgw.matchBucket), and every test that stores an object writes
	// `avatars/…`, not `/avatars/…`. Only this function used to keep the
	// bucket path's leading `/`, and the disagreement was invisible on
	// LOCAL_FS because filepath.Join collapses it; on memory / Redis /
	// S3 it is simply a different key, so an object written through the
	// upload path could not be read back through the download path.
