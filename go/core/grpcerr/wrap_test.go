...
// A class-22 data exception is the CALLER's value, not a server fault.
//
// It used to fall through to the unknown-error branch: codes.Internal
// plus an observx exception, so an over-long string answered a 500 and
// paged whoever watches Sentry. There is no constraint name on a 22001,
// so the registry cannot help and the message stays generic — but the
// CODE is unambiguous, and it is the code a client branches on.
func TestWrap_PgDataException_IsTheCallersValue(t *testing.T) {
	for _, tc := range []struct{ code, name string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			got := Wrap(context.Background(), "M.G",
				&pgconn.PgError{Code: tc.code, Message: "value too long for type character varying(64)"},
				nil, DialectPostgres)
			st, _ := status.FromError(got)
			if st.Code() != codes.InvalidArgument {
				t.Errorf("Code = %v, want InvalidArgument", st.Code())
			}
			// The driver's message names the column TYPE and the
			// offending length; neither belongs in a client-facing
			// string (principle 2 — no schema leak).
			if contains(got.Error(), "character varying") {
				t.Errorf("driver detail leaked to the client: %q", got.Error())
			}
		})
	}
}

// The rest of class 22 is NOT assumed to be the caller's. A division by
// zero is as likely to be the generated SQL's own arithmetic, so it
// keeps the internal path — where it is reported rather than answered
// as a 400 the client cannot act on.
func TestWrap_PgOtherClass22_StaysInternal(t *testing.T) {
	var got error
	withCapturedLog(t, func() {
		got = Wrap(context.Background(), "M.G",
			&pgconn.PgError{Code: "22012", Message: "division by zero"},
			nil, DialectPostgres)
	})
	st, _ := status.FromError(got)
	if st.Code() != codes.Internal {
