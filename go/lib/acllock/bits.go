...
// IDByString returns the allocated ID for a permission, or (0, false) when it
// is not in the lock. Used by auth backends that consume permissions as
// strings (typical JWT claim shape) and need IDs for AuthResp.permission_ids.
//
...
func IDByString(lock *Lock, perm string) (int, bool) {
	...
	if id, ok := lock.Permissions[perm]; ok {
		return id, true
	}
	id, ok := lock.Permissions[PermissionKey(perm)]
	return id, ok
}
