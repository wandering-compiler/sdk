...
func TestClientStream_RecvMsg_EOFAndError(t *testing.T) {
	// Finished with no error → io.EOF. (The end of a stream is signalled by
	// its own channel, not by closing the one the server sends on — closing
	// that under a late sender is a process-killing panic.)
	p := &inprocStream{clientCtx: context.Background(), s2c: make(chan protomsg), finished: make(chan struct{})}
	close(p.finished)
	if err := (clientStream{p: p}).RecvMsg(wrapperspb.String("")); err != io.EOF {
		t.Errorf("RecvMsg on closed stream = %v, want io.EOF", err)
	}
	// Finished WITH an error → that error.
	p2 := &inprocStream{clientCtx: context.Background(), s2c: make(chan protomsg), finished: make(chan struct{}), err: errors.New("boom")}
	close(p2.finished)
