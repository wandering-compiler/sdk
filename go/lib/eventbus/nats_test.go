...
func TestNatsSubscriber_AdvisoryRespectsDraining(t *testing.T) {
	t.Run("draining short-circuits", func(t *testing.T) {
		...
		s.draining = true
		s.handleAdvisory(context.Background(), "d", []byte("{}"))
	...
	t.Run("non-draining proceeds", func(t *testing.T) {
		...
		s.handleAdvisory(context.Background(), "d", []byte("not-json"))
...
func TestNatsSubscriber_RouteToDLQ_BrokerFreeGuards(t *testing.T) {
	t.Run("valid advisory missing stream_seq", func(t *testing.T) {
		...
		s.routeToDLQ(context.Background(), "d", []byte(`{"stream":"default"}`))
	...
	t.Run("in-flight sequence deferred", func(t *testing.T) {
		obs := &countingObserver{}
		s := &natsSubscriber{bus: &NatsBus{opts: NatsBusOptions{Observer: obs}}, channel: "default"}
		defer s.markInFlight("dur", 42)() // a handler is still on seq 42
		s.routeToDLQ(context.Background(), "dur", []byte(`{"stream":"default","stream_seq":42}`))
		if obs.deliverFailures != 0 || obs.drops != 0 {
			t.Errorf("in-flight seq must defer (no failure, no drop); got %d/%d",
				obs.deliverFailures, obs.drops)
		}
	})
	// The guard belongs to the CONSUMER IDENTITY, not to the subscriber
	// object: a sibling subscriber on the same bus + durable is running the
	// same delivery, so its advisory must defer too. (Per-object maps left
	// each blind to the other and dead-lettered a message mid-flight.)
	t.Run("sibling subscriber on the same durable defers", func(t *testing.T) {
		obs := &countingObserver{}
		bus := &NatsBus{opts: NatsBusOptions{Observer: obs}}
		busy := &natsSubscriber{bus: bus, channel: "default"}
		sibling := &natsSubscriber{bus: bus, channel: "default"}
		defer busy.markInFlight("dur", 42)()
		if !sibling.inFlightBusy("dur", 42) {
			// Fatal, not Error: past the guard routeToDLQ reaches the broker,
			// which this broker-free test has none of.
			t.Fatal("a sibling on the same durable does not see its peer's " +
				"delivery as in flight — it would dead-letter a running handler's message")
		}
		sibling.routeToDLQ(context.Background(), "dur", []byte(`{"stream":"default","stream_seq":42}`))
		if obs.deliverFailures != 0 || obs.drops != 0 {
			t.Errorf("a sibling on the same durable must defer to the running handler; got %d/%d",
				obs.deliverFailures, obs.drops)
		}
	})
	// ...and no further: a DIFFERENT durable on the same stream has its own
	// delivery budget, so its exhausted message must still be routed. Skipping
	// it because someone else holds the same sequence would lose the poison
	// message entirely — the advisory is one-shot. (Asserted on the guard
	// itself: proceeding past it reaches the broker, which this broker-free
	// test has none of.)
	t.Run("different durable is not skipped", func(t *testing.T) {
		bus := &NatsBus{opts: NatsBusOptions{Observer: &countingObserver{}}}
		busy := &natsSubscriber{bus: bus, channel: "default"}
		other := &natsSubscriber{bus: bus, channel: "default"}
		release := busy.markInFlight("dur-a", 42)
		if !other.inFlightBusy("dur-a", 42) {
			t.Error("a sibling on the SAME durable must see the delivery as in flight")
		}
		if other.inFlightBusy("dur-b", 42) {
			t.Error("a DIFFERENT durable must not be skipped — its own exhausted " +
				"delivery would never reach the DLQ")
		}
		if other.inFlightBusy("dur-a", 43) {
			t.Error("only the marked sequence is in flight")
		}
		release()
		if other.inFlightBusy("dur-a", 42) {
			t.Error("the release func must clear the entry")
		}
	})
}
