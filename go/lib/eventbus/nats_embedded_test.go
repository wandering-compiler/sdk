...
// topicOutcomes counts the deliver-success and drop records for one topic —
// the A-P9-1 tests must assert PER MESSAGE coherence (one message must never
// be reported both dropped and delivered), which the aggregate snapshots()
// cannot express.
func topicOutcomes(obs *recordingObserver, topic string) (successes, drops int) {
	obs.mu.Lock()
	defer obs.mu.Unlock()
	for _, r := range obs.deliverSuccess {
		if r.Topic == topic {
			successes++
		}
	}
	for _, r := range obs.drop {
		if r.Topic == topic {
			drops++
		}
	}
	return successes, drops
}

// INVARIANT (A-P9-1, prefetch variant): a message that sits in the client's
// prefetch buffer past its AckWait at the delivery cap — because an earlier
// message's handler is hogging the consume callback — must reach exactly ONE
// terminal outcome. The server fires its MAX_DELIVERIES advisory while the
// message is buffered and unstarted, so the process-local in-flight mark does
// not exist yet; without a broker-shared claim the advisory dead-letters the
// message IN THE VERY PROCESS that then runs its handler successfully — one
// message reported both dropped and delivered.
func TestNatsBus_AdvisoryWhileBuffered_NoDropPlusSuccess_Embedded(t *testing.T) {
	obs := &recordingObserver{}
	bus, err := eventbus.NewNatsBus(eventbus.NatsBusOptions{
		DSN:               startEmbeddedNATS(t),
		DurablePrefix:     "ap91-buffered",
		DefaultMaxDeliver: 1,
		DefaultAckWait:    400 * time.Millisecond,
		Observer:          obs,
	})
	...
	if err := sub.Subscribe(context.Background(), "**",
		func(_ context.Context, topic string, _ []byte) error {
			switch topic {
			case "blocker":
				startedOnce.Do(func() { close(blockerStarted) })
				// Hog the (serial) consume callback well past the buffered
				// victim's AckWait, then succeed.
	...
	if err := bus.Dispatch(context.Background(), "default", "blocker", wrapperspb.String("slow")); err != nil {
	...
	// Delivered immediately into the consume buffer; its AckWait clock runs
	// while the blocker's handler keeps the callback busy.
	if err := bus.Dispatch(context.Background(), "default", "victim", wrapperspb.String("buffered")); err != nil {
		t.Fatalf("Dispatch victim: %v", err)
	}

	waitFor(t, 10*time.Second, "victim reaches a terminal outcome", func() bool {
		s, d := topicOutcomes(obs, "victim")
		return s+d >= 1
	})
	// Let the OTHER half of a (wrong) dual outcome land: the blocker finishes
	// at ~1.5s and the buffered victim's handler would run right after.
	time.Sleep(2500 * time.Millisecond)

	s, d := topicOutcomes(obs, "victim")
	if s > 0 && d > 0 {
		t.Errorf("victim reported BOTH delivered (%d) and dropped (%d) — A-P9-1 incoherent drop+success (victim handler ran %d times)",
			s, d, victimRuns.Load())
	}
	if s+d == 0 {
		t.Error("victim reached no terminal outcome at all")
	}
	if _, bd := topicOutcomes(obs, "blocker"); bd != 0 {
		t.Errorf("blocker (handler mid-run past AckWait) was dropped %d times; want 0 (D1)", bd)
	}
}

// INVARIANT (A-P9-1, cross-replica variant): the MAX_DELIVERIES advisory
// rides a cluster-wide queue group keyed on the shared durable name, so it
// can land on a replica that never marked the message in flight while the
// handler is still running on its sibling. Each message must still reach
// exactly one terminal outcome. Two buses sharing one DurablePrefix model the
// two replicas; every handler overruns AckWait on its only delivery and then
// succeeds, so on unfixed code every advisory that crosses replicas
// dead-letters a message that also completes successfully (~half of the
// rounds; the test needs just one of 8 to cross). The claim-based guard makes
// every round success-only, deterministically.
func TestNatsBus_CrossReplicaAdvisory_NoDropPlusSuccess_Embedded(t *testing.T) {
	url := startEmbeddedNATS(t)
	const prefix = "ap91-xreplica"
	const rounds = 8

	newReplica := func() *recordingObserver {
		obs := &recordingObserver{}
		bus, err := eventbus.NewNatsBus(eventbus.NatsBusOptions{
			DSN:               url,
			DurablePrefix:     prefix,
		...
		if err := sub.Subscribe(context.Background(), "**",
			func(_ context.Context, _ string, _ []byte) error {
				time.Sleep(900 * time.Millisecond) // overrun AckWait, then succeed
				return nil
			}); err != nil {
			t.Fatalf("Subscribe: %v", err)
		}
	...
	outcomes := func(topic string) (successes, drops int) {
		sa, da := topicOutcomes(obsA, topic)
		sb, db := topicOutcomes(obsB, topic)
		return sa + sb, da + db
	}

	// Sequential rounds so each advisory fires while its message's handler is
	// MID-RUN on one replica (never merely buffered behind a predecessor —
	// that is the other variant's test).
	for i := 0; i < rounds; i++ {
		topic := fmt.Sprintf("m%d", i)
		if err := pubBus.Dispatch(context.Background(), "default", topic, wrapperspb.String(topic)); err != nil {
			t.Fatalf("Dispatch %s: %v", topic, err)
		}
		waitFor(t, 10*time.Second, topic+" reaches a terminal outcome", func() bool {
			s, d := outcomes(topic)
			return s+d >= 1
		})
		// Let the other half of a (wrong) dual outcome land — a drop
		// surfaces at ~AckWait, the handler's success at ~900ms.
	...
	for i := 0; i < rounds; i++ {
		...
		total += s + d
		if s > 0 && d > 0 {
			t.Errorf("%s reported BOTH delivered (%d) and dropped (%d) — A-P9-1 incoherent drop+success across replicas", topic, s, d)
		}
	}
	if total < rounds {
		t.Errorf("only %d terminal outcomes across %d rounds; every message must reach one", total, rounds)
	}
}
