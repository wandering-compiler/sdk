...
func HandleW17Events(w http.ResponseWriter, r *http.Request, source EventSource, heartbeat time.Duration) {
	...
	// A parked SSE client has nothing to drain, but http.Server.Shutdown
	// can't know that — it counts the connection as an active request and
	// waits out the whole graceful window before the transport gives up
	// and force-closes. So the connection ends itself the moment shutdown
	// starts: the drain signal is closed at t=0 of the window (see
	// lifecycle.BindHTTP), the client sees a clean EOF and reconnects to
	// the next instance per EventSource semantics. Nil — and therefore
	// never ready — when the handler runs outside a bound server.
	draining := lifecycle.Draining(ctx)
