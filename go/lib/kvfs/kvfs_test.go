...
// BuildKey returns a key with NO leading slash — the form every driver
// and every caller uses. These cases used to pin the opposite, which is
// how the outlier survived: the write path stored `/avatars/…` while
// restgw's download read path reconstructed `avatars/…`, so an object
// written through an upload could not be read back through a download.
// Invisible on LOCAL_FS (filepath.Join collapses the slash) and a plain
// key mismatch on memory / Redis / S3.
...
func TestBuildKey_Distribution(t *testing.T) {
	...
	for i := 0; i < nKeys; i++ {
		...
		// the object key.
		parts := strings.Split(strings.TrimPrefix(built, strings.TrimPrefix(bucketRoot, "/")+"/"), "/")
		if len(parts) < 2 {
