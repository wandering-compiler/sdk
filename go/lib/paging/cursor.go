...
// ErrCursorExpired indicates the cursor's `schema_version` is not
// this endpoint's. Two causes, indistinguishable from the token
// alone: the cursor was emitted by a server build whose request
// schema (or KeysetValue shape) has since changed, or it was minted
// by a DIFFERENT endpoint — the hash covers the request message's
// full name (see gateway/pagedemit.SchemaVersionOf), so one list's
// cursor never validates on another's. Remediation is the same for
// both (drop the token, refetch page one), which is why they share
// an error; handler messages must not assert one cause over the
// other.
//
