...
func TestMemory_Begin_RoundTrip(t *testing.T) {
	...
	tx, release, err := reg.LookupTx(context.Background(), id, "main")
	if err != nil || tx == nil {
	...
	// Cleanup so the test doesn't leak the open tx: end the adoption
	// lease first, or the finisher waits for it.
	release()
	_ = reg.Rollback(context.Background(), id)
}
...
func TestMemory_Commit_RemovesTx(t *testing.T) {
	...
	tx, release, _ := reg.LookupTx(context.Background(), id, "main")
	...
	if _, _, err := reg.LookupTx(context.Background(), id, "main"); !errors.Is(err, txregistry.ErrUnknownTxID) {
...
func TestMemory_UnknownID(t *testing.T) {
	reg := singleConnRegistry(t)
	if err := reg.Commit(context.Background(), "does-not-exist"); !errors.Is(err, txregistry.ErrUnknownTxID) {
		t.Errorf("Commit(unknown) = %v, want ErrUnknownTxID", err)
	}
	if err := reg.Rollback(context.Background(), "does-not-exist"); !errors.Is(err, txregistry.ErrUnknownTxID) {
		t.Errorf("Rollback(unknown) = %v, want ErrUnknownTxID", err)
	}
	if _, _, err := reg.LookupTx(context.Background(), "does-not-exist", "main"); !errors.Is(err, txregistry.ErrUnknownTxID) {
...
func TestMemory_DoubleClose(t *testing.T) {
	...
	if err := reg.Rollback(context.Background(), id); !errors.Is(err, txregistry.ErrUnknownTxID) {
...
func TestMemory_LookupTx_ConnectionMismatch(t *testing.T) {
	...
	defer func() { _ = reg.Rollback(context.Background(), id) }()
	if _, _, err := reg.LookupTx(context.Background(), id, "audit"); !errors.Is(err, txregistry.ErrConnectionMismatch) {
		t.Errorf("LookupTx with wrong connection = %v, want ErrConnectionMismatch", err)
	}
	// Same id resolves on the right connection.
	_, release, err := reg.LookupTx(context.Background(), id, "main")
	if err != nil {
...
func TestMemory_MultiConnection_BeginsRoute(t *testing.T) {
	...
	txMain, releaseMain, err := reg.LookupTx(context.Background(), idMain, "main")
	if err != nil {
	...
	txAudit, releaseAudit, err := reg.LookupTx(context.Background(), idAudit, "audit")
	if err != nil {
	...
	if _, _, err := reg.LookupTx(context.Background(), idMain, "audit"); !errors.Is(err, txregistry.ErrConnectionMismatch) {
		t.Errorf("idMain on audit = %v, want ErrConnectionMismatch", err)
	}
	releaseMain()
	releaseAudit()
	if err := reg.Commit(context.Background(), idMain); err != nil {
		t.Fatalf("Commit main: %v", err)
	}
	if err := reg.Commit(context.Background(), idAudit); err != nil {
	...
	for iter := 0; iter < 30; iter++ {
		...
		for i := 0; i < racers; i++ {
			go func() {
				defer wg.Done()
				<-start
				results <- reg.Commit(context.Background(), id)
			}()
		}
		close(start)
		wg.Wait()
...
func TestMemory_ConcurrentLookupVsCommit(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		<-start
		commitDone <- reg.Commit(context.Background(), id)
	}()
	...
	for i := 0; i < lookupers; i++ {
		go func() {
			...
			// Three legal answers now, not two: a live tx, an id the
			// winning Commit already took (ErrUnknownTxID), or the
			// adoption lease held by a sibling / claimed by the
			// waiting Commit (ErrTxBusy). Never a torn state.
			if err != nil && !errors.Is(err, txregistry.ErrUnknownTxID) && !errors.Is(err, txregistry.ErrTxBusy) {
