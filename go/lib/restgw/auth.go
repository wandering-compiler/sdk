...
//
// The slice belongs to this request: with the auth cache enabled
// ([CachedAuthFunc]) each hit is handed its own copy, so writing
...
// authCacheMaxEntries bounds how many credentials the auth cache holds at
// once. Without a bound the cache was a memo table with no exit: an entry
// was reclaimed ONLY by a later `get` on the same key finding it expired,
...
// caps the cache around 8 MB in the worst case and well under that in
// practice. Overshooting it costs a cache MISS — one inner auth call, the
// same work an unconfigured gateway does on every request — never a wrong
// or stale principal. It is deliberately not env-configurable: the cache is
// already opt-in per surface (`<PREFIX>_AUTH_CACHE_TTL_SECONDS`, unset =
// no cache at all), and a second knob to get wrong buys nothing a shorter
// TTL does not.
const authCacheMaxEntries = 8192

// authCacheTargetEntries is the low-water mark an eviction sweeps down to.
// Evicting back to exactly the ceiling would put the NEXT insert over it
// again, so every subsequent miss would pay for a full sweep; halving makes
// a sweep amortise over the inserts that refill the gap.
const authCacheTargetEntries = authCacheMaxEntries / 2

// authCache is the in-memory TTL store for [CachedAuthFunc].
// sync.Map gives lock-free reads + amortised inserts; the
// per-entry expiry is checked on get, and insertions past
// [authCacheMaxEntries] trigger an inline sweep, so no goroutine reaper is
// needed — which matters because this cache is built inside a closure with
// no lifecycle: a reaper goroutine here would have nothing to stop it.
type authCache struct {
	m   sync.Map // map[string]authEntry
	ttl time.Duration
	// n is the live entry count. It is EXACT for entries that went through
	// put/drop (both act on sync.Map's authoritative loaded/replaced flag)
	// and only approximate if something stores into m directly, which only
	// this package's own tests do. A transient overshoot is possible while
	// one goroutine sweeps and others insert — the ceiling is a bound on
	// steady state, not a mutex.
	n atomic.Int64
	// evicting keeps one sweeper at a time. A second putter that finds a
	// sweep running skips its own rather than duplicating the Range: the
	// sweep in flight is already removing what it would remove.
	evicting atomic.Bool
}

// authEntry's data belongs to the CACHE and is never handed out: `get`
// returns a copy and `put` stores one. It used to hand the same backing
// array to every hit — and to keep the inner AuthFunc's own slice — under a
// read-only contract that a doc comment stated and nothing enforced, so a
// single consumer writing through the slice it was given rewrote the
// principal every other concurrent request for that credential was about to
// read. One clone per hit is far cheaper than the inner auth RPC the hit
// avoids, which is the trade the cache exists to make in the first place.
...
func (c *authCache) get(key string) ([]byte, bool) {
	...
	return bytes.Clone(e.data), true
}

func (c *authCache) put(key string, data []byte) {
	if _, replaced := c.m.Swap(key, authEntry{
		data:   bytes.Clone(data),
		expiry: time.Now().Add(c.ttl),
	}); !replaced {
		c.n.Add(1)
	}
	if c.n.Load() > authCacheMaxEntries {
		c.evict()
	}
}

// drop removes a key and keeps the counter honest. The delete has to report
// whether it actually removed something — decrementing on a key another
// goroutine already dropped would drift the count downwards and disarm the
// ceiling.
func (c *authCache) drop(key string) {
	if _, loaded := c.m.LoadAndDelete(key); loaded {
		c.n.Add(-1)
	}
}

// evict brings the cache back under [authCacheTargetEntries]: expired
// entries first (they are pure waste, and reclaiming them is what the
// missing sweeper never did), then arbitrary live ones if expiry alone was
// not enough. Evicting a live entry is safe by construction — it costs the
// next request for that credential one inner auth call.
func (c *authCache) evict() {
	...
	c.m.Range(func(k, v any) bool {
		if e, ok := v.(authEntry); ok && now.After(e.expiry) {
			c.drop(k.(string))
		}
		return c.n.Load() > authCacheTargetEntries
	})
	// Every entry is still live and we are still over the ceiling: shed
	// down to the low-water mark. sync.Map's Range order is unspecified,
	// so the victims are arbitrary — deliberately, since the alternative
	// (an LRU) needs a per-hit write on the read path the cache exists to
	// keep lock-free.
	c.m.Range(func(k, _ any) bool {
		if c.n.Load() <= authCacheTargetEntries {
			return false
		}
		c.drop(k.(string))
		return true
